...
def debug_output( msg, indent=6 ):
    print('%s{WSSD %s} [%s] %s'%( ''.join(' '*indent), _cur_hostname, time.ctime(), msg ), file=sys.stderr)
...
#            from IPython.Shell import IPShellEmbed; ipshell = IPShellEmbed([]); ipshell()
    ...
    def populateCurChunkHits(self,
        ...
        for ir in range(lStartsClipped.shape[0]):
    ...
    def populateFromSAMFilesChunkify(self, 
        ...
        while any( [len(lNextLine[ifile])>0 and lNextLine[ifile][0]=='@' for ifile in range(len(lFns))] ):
            for ifile in range(len(lFns)):
...
class WssdFile(DenseTrackSet):
    ...
    def populateCurChunkHits(self,
        ...
        for ir in range(lStartsInChunk.shape[0]):
...
def findBoundariesForAboutEqualWeightedWindowsMaxAndMinWidthEnforcedBreaks( 
    ...
    for jbrkCheck in range(lcoBreaksCheck.shape[0]):
...
def extractPerBase(
    ...
    for iq in range(len(lchromQuery)):
...
def extractPerWnd( 
    ...
    for iq in range( len(lchromQuery) ):
        ...
        for iwnd in range( lcorngWndBnds.shape[0] ):
...
def extractPerVarWndQuickDefinedWindows( 
    ...
    for iw in range(lofsWnds.shape[0]):
