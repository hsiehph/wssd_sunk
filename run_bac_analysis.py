...
class GC_correction:
    ...
    def add_wssd_file(self,wssd,bac,wssd_dim):
        ...
        for n_gc,sum_depth in self.sum_read_depth.items():
    ...
    def get_and_output_udepth_gc(self,fn):
        ...
        #print GC_width
        correction_tuple=kgf.get_GC_depth_correction_vect(list(self.sum_read_depth.values()),list(self.n_locations.values()),GC_width)
...
def calc_gc_correction(indiv_lib_output_dir,indiv,bac_wssd,hg_wssd,control_bacs,GC_width,wssd_dim):
    ...
    for bac_name,bac in control_bacs.items():
        if(bac.cp==2):
            if(bac.source == 'hg18'):
...
def make_bac_plots(bacs,wssd,analysis_dir_out,mask,mask_grps):
    ...
    sp = 1
    for bac_name,bac in bacs.items():
